...
const AdCard: React.FC<componentProps> = ({adImage, type, format})=>{
    ...
    async function triggerDownloadImage(imageUrl: string, defaultFileName: string = 'downloaded-image') {
        setSaveMessage({
            message: 'Downloading...',
            error: false
        })
            ...
            }
            {
                type === "search" ? (
                    <div className='w-full h-max p-1 px-1 bg-[#f5f5f5] text-sm truncate rounded-b-md flex items-center justify-between'>
                        ...
                        <div onClick={()=> triggerDownloadImage(adImage, 'Ads by AdsInspect')} className='w-max p-1 rounded-md hover:bg-[#ffff]'>
